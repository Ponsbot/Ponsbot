...
const commands = [
  ...
  },
  {
    ...
    body: "Start with a token name and ticker. You can also include artwork, a description, website, X account, an available pairing asset, and an optional developer buy. Ponsbot collects and validates the launch details before the Pons V2 launch proceeds.",
    tips: ["A token name and ticker are required.", "Attach artwork to the same X post when you want custom token art.", "Pairing assets and developer buys must meet current Pons V2 rules."],
...
export default function HowItWorks() {
  return <main><SiteHeader /><section className="how-page">
    ...
    <section className="launch-guide"><div><p className="eyebrow">Launch on Pons V2</p><h2>Start simple. Add the details that make it yours.</h2></div><div className="launch-detail-list"><p><strong>Required</strong><span>Choose a token name and ticker, such as Ponsbot and $PONSBOT.</span></p><p><strong>Make it recognizable</strong><span>Attach artwork and include a concise description in the launch post.</span></p><p><strong>Connect your community</strong><span>Add the token website and X account so people can find the official links.</span></p><p><strong>Choose the market</strong><span>Select a currently supported pairing asset, such as MSFT when available, and optionally include a developer buy.</span></p></div></section>
