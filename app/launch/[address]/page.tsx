...
export default async function LaunchPage({ params }: Props) {
  ...
  const summary = <div className="detail-copy"><h1>{launch.name}</h1><p className="ticker">${launch.symbol}</p><div className="token-summary-address"><CopyAddress address={launch.tokenAddress} /></div><p className="description">{launch.description || "\u00a0"}</p>
    <div className="detail-socials">{launch.website ? <a className="button button-quiet" href={launch.website} target="_blank" rel="noreferrer">Website ↗</a> : null}{launch.twitter ? <a className="button button-quiet social-x-button" href={launch.twitter} target="_blank" rel="noreferrer" aria-label="X page"><Image src="/x-logo.png" alt="" width={14} height={14} /></a> : null}{launch.telegram ? <a className="button button-quiet" href={launch.telegram} target="_blank" rel="noreferrer">Telegram ↗</a> : null}</div>
    <div className="detail-actions"><a className="button button-dark" href={`https://www.ponsfamily.com/launchpad/${launch.tokenAddress}`} target="_blank" rel="noreferrer">View Token on Pons ↗</a></div></div>;
