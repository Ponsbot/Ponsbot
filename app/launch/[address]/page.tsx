...
export default async function LaunchPage({ params }: Props) {
  ...
  const summary = <div className="detail-copy"><h1>{launch.name}</h1><p className="ticker">${launch.symbol}</p><p className="description">{launch.description || "\u00a0"}</p>
  ...
  return <main><SiteHeader /><section className="detail-shell"><Link className="back-link" href="/#launches">← All launches</Link><div className="detail-grid">
    <div className="detail-left"><div className="detail-art">{launch.imageUri ? <Image src={launch.imageUri} alt={`${launch.name} token`} width={960} height={960} sizes="(max-width: 900px) 100vw, 480px" priority /> : <Image className="detail-art-placeholder" src="/ponsbot.png" alt="Pons Bot placeholder" width={960} height={960} sizes="(max-width: 900px) 100vw, 480px" priority />}{launch.graduated ? <strong className="graduated-badge">Graduated</strong> : null}{launch.marketCapUsd !== undefined ? <strong className="detail-art-mcap">MCap {formatMarketCap(launch.marketCapUsd)}</strong> : null}</div>
    ...
    </div><TokenActivity tokenAddress={launch.tokenAddress} symbol={launch.symbol} summary={summary} /></div></section><SiteFooter /></main>;
}
