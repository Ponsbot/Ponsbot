...
export function LaunchGrid({ launches }: { launches: PublicLaunch[] }) {
  ...
  const choose = (next: Sort) => { setSort(next); setPage(1); };
  ...
  const receiveRecent = useCallback((incoming: PublicLaunch[], cursor: string | null) => { updateLaunches(incoming); setNextCursor(cursor); }, [updateLaunches]);
  const loadMore = async () => {
    if (!nextCursor || loadingMore) return;
    setLoadingMore(true);
    try {
      const payload = await fetch(`/api/launches?count=40&cursor=${encodeURIComponent(nextCursor)}`, { cache: "no-store" }).then((response) => response.ok ? response.json() : undefined);
  ...
  return <>
    <MarketViewer tokenAddresses={visible.flatMap((launch) => launch.tokenAddress ? [launch.tokenAddress] : [])} onMarket={updateMarket} onLaunches={receiveRecent} />
    <div className="launch-toolbar" aria-label="Sort launches">{([['newest','Newest'],['oldest','Oldest'],['mcap','Highest MCap'],['volume','24h Volume'],['lastBuy','Last Buy']] as const).map(([value,label]) => <button key={value} className={sort === value ? "active" : ""} type="button" onClick={() => choose(value)}>{label}</button>)}<button className={graduatedOnly ? "active" : ""} type="button" aria-pressed={graduatedOnly} onClick={() => { setGraduatedOnly((value) => !value); setPage(1); }}>Graduated</button></div>
...
function MarketViewer({ tokenAddresses, onMarket, onLaunches }: { tokenAddresses: string[]; onMarket: (market: MarketUpdate[]) => void; onLaunches: (launches: PublicLaunch[], cursor: string | null) => void }) {
  ...
  useEffect(() => {
    ...
    const ping = async () => {
      ...
      const [marketPayload, launchPayload] = await Promise.all([
        ...
        fetch("/api/launches?count=40", { cache: "no-store" }).then((response) => response.ok ? response.json() : undefined).catch(() => undefined),
      ]);
