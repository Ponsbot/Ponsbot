...
export function LegalDocument({ eyebrow, title, summary, effective, sections }: { eyebrow: string; title: string; summary: string; effective: string; sections: LegalSection[] }) {
  return <main><SiteHeader /><article className="legal-page">
    <Link className="back-link" href="/">← Home</Link>
    <header className="legal-hero"><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p>{summary}</p><small>Effective {effective}</small></header>
