...
export function TerminalClient() {
  ...
  return <section className="terminal-shell">
    <header className="terminal-heading">
      ...
      <div className="terminal-heading-title-row">
        <h1>Welcome, @{session.username}</h1>
        <div className="terminal-heading-actions"><div className="terminal-workspace-toggle" aria-label="Terminal workspace"><button type="button" className={workspace === "terminal" ? "active" : ""} onClick={() => setWorkspace("terminal")}>Terminal</button>{session.houdiniPreviewEnabled ? <button type="button" className={workspace === "houdini" ? "active" : ""} onClick={() => setWorkspace("houdini")}>Multi-Chain and Private Swaps</button> : null}<button type="button" className={workspace === "liquidity" ? "active" : ""} onClick={() => setWorkspace("liquidity")}>Liquidity Positions</button></div></div>
      </div>
      <p className="terminal-heading-description">{workspace === "houdini" ? "Cross-chain and private swaps from your Pons Bot wallet." : workspace === "liquidity" ? "Build and manage Delta Liquidity positions from your Pons Bot wallet." : "Buy, sell, swap, send, burn, claim creator fees, and manage Delta Liquidity positions directly from your connected wallet."}</p>
    </header>
