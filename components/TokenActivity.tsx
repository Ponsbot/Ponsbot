...
export function TokenActivity({ tokenAddress, symbol, artwork, summary, details }: { tokenAddress: string; symbol: string; artwork: ReactNode; summary: ReactNode; details: ReactNode }) {
  ...
  return <section className="token-market-panel"><div className="token-page-columns"><div className="token-left-column"><div className="token-market-overview"><div className="token-market-artwork">{artwork}</div><div className="token-market-summary">{summary}</div></div><div className="token-details-column">{details}</div></div><div className="token-right-column"><div className="token-chart-column"><div className="market-chart-head"><div><small>Market cap</small><strong>{points.length ? formatMoney(points[points.length - 1].marketCapUsd) : "—"}</strong></div><div className="chart-ranges">{(["1H", "6H", "1D"] as const).map((value) => <button type="button" className={range === value ? "active" : ""} onClick={() => setRange(value)} key={value}>{value}</button>)}</div></div><MarketChart points={points} /></div><div className="token-activity-column">
