  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      if (!stopped) {
        ...
        setLiveUpdatesAvailable(marketResult.ok);
        const nextMarketCap = marketResult.payload?.market?.find((item: { tokenAddress?: string }) => item.tokenAddress?.toLowerCase() === tokenAddress.toLowerCase())?.marketCapUsd;
