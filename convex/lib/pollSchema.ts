...
export const pollMinimumValidator = v.object({ unit: v.union(v.literal('usd'), v.literal('tokens')), amount: v.string() });
export const resolvedPollMinimumValidator = v.object({ balance: v.string(), percent: v.number(), pricedAt: v.optional(v.number()), marketCapUsd: v.optional(v.string()) });
export const pollSpecValidator = v.object({ token: v.string(), question: v.string(), options: v.array(v.string()), durationMinutes: v.number(), minimumHoldingPercent: v.number(), minimumHolding: v.optional(pollMinimumValidator) });
...
export const pollTables = {
  pollDraftTurns: defineTable({ postId: v.string(), owner: v.string(), expiresAt: v.number(), consumedBy: v.optional(v.string()), creationPostId: v.optional(v.string()),
    draft: v.object({ token: v.optional(v.string()), question: v.optional(v.string()), options: v.optional(v.array(v.string())), durationMinutes: v.optional(v.number()), minimumHoldingPercent: v.optional(v.number()), minimumHolding: v.optional(pollMinimumValidator) }),
  ...
  polls: defineTable({ code: v.string(), requestKey: v.string(), ownerXUserId: v.string(), creatorWallet: v.string(), creatorXUsername: v.optional(v.string()), source: v.union(v.literal('x'), v.literal('web')), sourcePostId: v.optional(v.string()),
    spec: pollSpecValidator, resolvedMinimum: v.optional(resolvedPollMinimumValidator), tokenAddress: v.optional(v.string()), status: v.union(v.literal('preparing'), v.literal('needs_token'), v.literal('open'), v.literal('closed'), v.literal('failed'), v.literal('cancelled')),
