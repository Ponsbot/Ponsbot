...
export const walletIntentSchema: JsonSchemaResponseFormat = {
  ...
  schema: {
    ...
    properties: {
      ...
      operation: {
        type: ["string", "null"],
        enum: [null, "create_wallet", "show_wallet", "show_balance", "send", "burn", "buy", "buy_and_send", "buy_and_burn", "swap_token_for_token", "sell", "claim_fees", "reassign_fees", "upgrade_fees", "launch"],
        description: "Command operation, or null unless kind is command.",
      },
      topic: {
        type: ["string", "null"],
        enum: [null, "capabilities", "wallet", "fund", "gas", "balance", "send", "buy_sell", "cross_chain", "cross_chain_assets", "burn", "launch", "pairs", "fees"],
...
const operationProperties: Record<string, Record<string, unknown>> = {
  ...
  reassign_fees: { token: nullableString("Ticker or contract from the user's Pons Bot launches."), recipient: nullableString("X handle, wallet address, or the literal word holders following 'to'.") },
