...
/** Stored percentage is of the creator allocation AFTER the existing 5% PONSBOT split.
...
export function creatorFeeBurnDisplay(symbol: string, policy?: CreatorSelfBurnDisplay, holderFeeSharing = false, tokenAddress?: string) {
  ...
  return bps === 10_000
    ...
    : { fullAllocation: null, suffix: `(${isPonsbotHalfTotal(tokenAddress, bps) ? 50 : bps / 100}% buyback and burn ${ticker})` };
}
