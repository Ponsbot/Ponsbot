...
export async function runAgentModel(kind: "thought" | "trade", raw: AgentMarketContext, signal: AbortSignal, call: AgentModelCall) {
  ...
  const result = await call([
    ...
    ...(kind === "trade" ? [{ role: "system" as const, content: "Diversify deliberately. Candidate order is rotated, not a ranking or recommendation. Compare at least three priced alternatives when available, including tokens not already held. Prefer opening a different suitable position over repeatedly buying the same token. Use balances, decimals and supplied prices to assess concentration: avoid adding to a token already dominating token holdings unless supplied evidence makes it clearly preferable. Do not default to PONSBOT because of the platform name. Recent completed purchases are a reason to explore alternatives, not repeat them. Explain the specific selection and, if adding to an existing holding, why it is better than alternatives. Never buy an unpriced or unsuitable token just for variety, never invent market evidence, and hold when no alternative is convincing. Do not sell merely to force variety. Existing universe restrictions, secondary-asset quota, gas and spending limits still apply." }] : []),
