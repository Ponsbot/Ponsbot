...
import { renderSpriteDesign, spriteDesigns, type SpriteArchetype } from "./sprite-designs";
export type BotSprite = { version: 1 | 2; seed: number; archetype: SpriteArchetype; palette: number };
...
export function createBotSprite(name: string, description: string): BotSprite {
  ...
  const archetype: SpriteArchetype = /wizard|magic|mage|witch/.test(words) ? "wizard" : /cat|kitten|feline/.test(words) ? "cat"
    : /plant|flower|garden|tree|nature/.test(words) ? "plant" : /pirate|captain|sailor/.test(words) ? "pirate"
    : /space|astro|cosmic/.test(words) ? "astronaut" : /bird|owl|penguin/.test(words) ? "bird"
    : /jelly|ocean|ghost/.test(words) ? "jelly" : /rock|golem|giant/.test(words) ? "golem"
    : /rover|wheel|racer/.test(words) ? "rover" : (Object.keys(spriteDesigns) as SpriteArchetype[])[(seed >>> 8) % 10];
  return { version: 2, seed, archetype, palette: seed % palettes.length };
}
...
export function botSpriteDataUrl(sprite: BotSprite): string {
  if (![1, 2].includes(sprite.version) || !Number.isSafeInteger(sprite.seed) || sprite.seed < 0 || sprite.seed > 0xffffffff
    || !Number.isInteger(sprite.palette) || !palettes[sprite.palette] || !Object.hasOwn(spriteDesigns, sprite.archetype)) throw new Error("INVALID_BOT_SPRITE");
  const [body, accent] = palettes[sprite.palette];
  if (sprite.version === 2) return `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="25" height="30" viewBox="0 0 25 30" shape-rendering="crispEdges">${renderSpriteDesign(sprite.archetype, sprite.seed, body, accent)}</svg>`)}`;
