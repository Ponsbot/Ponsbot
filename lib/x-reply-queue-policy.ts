...
export function replyQueuePriority(text: string, kind?: string, ok?: boolean): ReplyPriority {
  ...
  if (/^(?:Your Pons Bot wallet|Here's your wallet balance|Here is your wallet balance|Ask\b|Tell me\b|Say\b|Verified (?:X )?accounts can (?:post|launch)|Claim with\b|You can pair|Hi there|I couldn't quite make that out|This request did not complete)/i.test(opening)) return "C";
