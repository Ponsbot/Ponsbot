...
describe("fixed schedules and economic threshold", () => {
  ...
  it("retains enrollment slots when checks and transaction completion are late", () => {
    ...
    expect(nextFeeCheck(p, 100_000)).toBe(18_100_000);
    expect(nextFeeCheck(p, 1_030_000)).toBe(18_100_000);
    expect(nextFeeCheck(p, 1_080_000)).toBe(18_100_000);
    expect(nextFeeCheck(p, 18_140_000)).toBe(36_100_000);
    expect(nextFeeCheck({ scheduleAnchorAt: 100_000, enrolledAt: 500_000 }, 1_080_000)).toBe(18_100_000);
  });
