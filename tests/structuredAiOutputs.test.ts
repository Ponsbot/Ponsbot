...
describe("OpenRouter structured wallet outputs", () => {
  ...
  it("defines closed schemas for every specialized operation", () => {
    const operations = ["create_wallet", "show_wallet", "show_balance", "send", "burn", "buy", "buy_and_send", "buy_and_burn", "swap_token_for_token", "sell", "claim_fees", "launch"];
  ...
  it("keeps the prompt aligned with schema-invalid and USD-sell behavior", () => {
    expect(parameterExtractorPrompt("sell", false)).toContain('unit":"usd|token|percent');
